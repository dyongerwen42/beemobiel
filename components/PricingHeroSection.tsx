  ...
  return (
    <section ref={ref} className="relative min-h-[35vh] sm:min-h-[40vh] md:min-h-[45vh] lg:min-h-[50vh] flex items-center justify-center overflow-hidden">
      ...
      <div className={`relative z-10 w-full max-w-7xl mx-auto px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8 py-8 sm:py-10 md:py-12 lg:py-14 text-center`}>
        <div className={`inline-flex items-center px-2.5 sm:px-3 md:px-3.5 py-1 sm:py-1.5 md:py-1.5 bg-yellow-50/90 backdrop-blur-sm rounded-full mb-1.5 sm:mb-2 md:mb-2.5 border border-yellow-200/60 shadow-md transition-opacity duration-700 ${isIntersecting ? 'opacity-100' : 'opacity-0'}`}>
          <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-yellow-600 rounded-full mr-1.5 sm:mr-2 animate-pulse"></div>
          <p className="text-yellow-700 uppercase font-bold text-xs sm:text-sm tracking-wider">Tarieven</p>
        </div>
        ...
        <p className={`text-xs sm:text-sm md:text-base text-white/90 max-w-2xl mx-auto leading-relaxed text-shadow-md transition-opacity duration-700 px-2 ${isIntersecting ? 'opacity-100' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
          Bij BeeMobiel kies je voor altijd lage, eerlijke tarieven. Met een breed aanbod aan verschillende pakketten voor autorijlessen, motorrijlessen en scooterrijlessen zorgen we ervoor dat iedereen vindt wat hij of zij zoekt.
        </p>
      </div>
    </section>
  )
